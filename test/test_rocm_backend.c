...
int main(void) {
#ifndef BN_ENABLE_ROCM
    printf("ROCm backend test skipped: BN_ENABLE_ROCM not set\n");
    return 0;
#else
    BnGPUBackend *gpu = bn_gpu_rocm_create();
    if (!gpu) {
        printf("ROCm backend test skipped: no ROCm device\n");
        return 0;
    }
    ...
    printf("ROCm backend test PASSED\n");
    return 0;
#endif
}
